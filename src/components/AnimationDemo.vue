      ...
      <div class="lottie-panel">
        <div v-if="mode === 'dactyl'" class="hand-pose-panel">
          <div class="pose-board">
            ...
            <div class="hand-stage">
              <div v-if="dactylVideoUrl" class="dactyl-video-panel">
                <video
                  ref="videoPlayer"
                  :src="dactylVideoUrl"
                  class="dactyl-video-player"
                  autoplay
                  loop
                  muted
                  playsinline
            ...
            <div v-if="!dactylVideoUrl" class="dactyl-image-missing">
              {{ t('dactyl_video_unavailable') }}
            </div>
          </div>
        ...
        <div v-else>
          ...
          <img
            ...
            :src="mediaUrl"
            class="animation-container image-preview"
            alt="Local media asset"
          />
        ...
        <div v-if="props.showPlaceholder" class="animation-placeholder">
          {{ t('assets_coming_soon') }}
        </div>
      </div>
    </div>
  </div>
...
const props = defineProps({
  ...
  mediaAsset: { type: Object, default: null },
...
const dactylVideoUrl = computed(() => {
  const letter = currentLetter.value
  if (!letter) return null
  return `/images/dactyl/${letter.toUpperCase()}.mp4`
})
...
const mediaUrl = computed(() => props.mediaAsset?.url || props.videoUrl)
const mediaType = computed(() => props.mediaAsset?.type || (props.videoUrl ? 'video' : null))

const loadVideo = async (sourceUrl) => {
  ...
  if (!videoPlayer.value || !sourceUrl) return

  try {
    videoPlayer.value.src = sourceUrl
...
const loadCurrentDactylVideo = async () => {
  if (!videoPlayer.value) return
  if (!dactylVideoUrl.value) {
    videoPlayer.value.src = ''
    return
  }

  try {
    videoPlayer.value.src = dactylVideoUrl.value
    await videoPlayer.value.play().catch(() => {})
  } catch (err) {
    // ignore
  }
...
const initAnimation = async () => {
  if (props.mode === 'dactyl') {
    await loadCurrentDactylVideo()
    return
  }

  if (mediaType.value === 'video') {
    await loadVideo(mediaUrl.value)
    return
  }
...
watch(() => currentLetter.value, async () => {
  if (props.mode === 'dactyl') {
    await loadCurrentDactylVideo()
  }
})
